...
public unsafe class HighResScreenshots : Tweak {
    ...
    protected void DrawConfig(ref bool changed)
    {
        ...
        changed |= ImGui.Checkbox("Use Fixed Resolution", ref Config.UseCustom);

        if (Config.UseCustom) {

            changed |= ImGui.InputInt("Width", ref Config.CustomWidth);
            changed |= ImGui.InputInt("Height", ref Config.CustomHeight);

        } else {
            ImGui.SetNextItemWidth(ImGuiHelpers.GlobalScale * 100);
            changed |= ImGui.InputInt("Scale", ref Config.Scale);
        ...
        ImGui.SetNextItemWidth(ImGuiHelpers.GlobalScale * 100);
        changed |= ImGui.InputFloat("Delay", ref Config.Delay);

        if (Config.Scale < 1) Config.Scale = 1;
        if (Config.Delay < 0) Config.Delay = 0;
        changed |= ImGui.Checkbox("Hide Dalamud UI in screenshots", ref Config.HideDalamudUi);
        changed |= ImGui.Checkbox("Hide Game UI in screenshots", ref Config.HideGameUi);
        if (copyrightShaderAddress == 0) {
            ...
            changed |= ImGui.Checkbox("Remove copyright text", ref Config.RemoveCopyright);
        }
        ...
        VirtualKey key = Service.KeyState.GetValidVirtualKeys().FirstOrDefault(k => k is not (VirtualKey.CONTROL or VirtualKey.SHIFT or VirtualKey.MENU) && Service.KeyState[k], VirtualKey.NO_KEY);
    }
    ...
    private byte IsInputIDClickedDetour(nint a1, int a2) {
        if (a2 == ScreenshotButton && Config.UseReShade && Framework.Instance()->WindowInactive) 
            return 0;
        ...
        if (orig == 1 && a2 == ScreenshotButton && !shouldPress && !isRunning) {
            ...
            if (Config.HideGameUi) {
                var raptureAtkModule = Framework.Instance()->UIModule->GetRaptureAtkModule();
